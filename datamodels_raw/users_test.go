...
func testUserToManyUserDomains(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := UserSlice{&a}
	if err = a.L.LoadUserDomains(ctx, tx, false, (*[]*User)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testUserToManyAddOpUserDomains(t *testing.T) {
	var err error
...
var (
	userDBTypes = map[string]string{`IDBase64`: `uuid`, `Username`: `character varying`, `Email`: `character varying`, `EncryptedPassword`: `bytea`, `SuperAdmin`: `boolean`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`}
	_           = bytes.MinRead
)
