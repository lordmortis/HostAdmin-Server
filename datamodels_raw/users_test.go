...
var (
	userDBTypes = map[string]string{`ID`: `uuid`, `Username`: `character varying`, `Email`: `character varying`, `EncryptedPassword`: `bytea`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`}
	_           = bytes.MinRead
)
