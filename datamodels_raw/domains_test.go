...
func testDomainToManyDomainEmailAliases(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, domainEmailAliasDBTypes, false, domainEmailAliasColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, domainEmailAliasDBTypes, false, domainEmailAliasColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.DomainEmailAliases().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := DomainSlice{&a}
	if err = a.L.LoadDomainEmailAliases(ctx, tx, false, (*[]*Domain)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.DomainEmailAliases); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.DomainEmailAliases = nil
	if err = a.L.LoadDomainEmailAliases(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.DomainEmailAliases); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testDomainToManyDomainEmailUsers(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, domainEmailUserDBTypes, false, domainEmailUserColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, domainEmailUserDBTypes, false, domainEmailUserColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.DomainEmailUsers().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := DomainSlice{&a}
	if err = a.L.LoadDomainEmailUsers(ctx, tx, false, (*[]*Domain)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.DomainEmailUsers); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.DomainEmailUsers = nil
	if err = a.L.LoadDomainEmailUsers(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.DomainEmailUsers); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testDomainToManyUserDomains(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, userDomainDBTypes, false, userDomainColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, userDomainDBTypes, false, userDomainColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.UserDomains().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := DomainSlice{&a}
	if err = a.L.LoadUserDomains(ctx, tx, false, (*[]*Domain)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.UserDomains); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.UserDomains = nil
	if err = a.L.LoadUserDomains(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.UserDomains); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testDomainToManyAddOpDomainEmailAliases(t *testing.T) {
	var err error
	...
	foreigners := []*DomainEmailAlias{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, domainEmailAliasDBTypes, false, strmangle.SetComplement(domainEmailAliasPrimaryKeyColumns, domainEmailAliasColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*DomainEmailAlias{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddDomainEmailAliases(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.DomainEmailAliases[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.DomainEmailAliases[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.DomainEmailAliases().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
func testDomainToManyAddOpDomainEmailUsers(t *testing.T) {
	var err error
	...
	var a Domain
	var b, c, d, e DomainEmailUser

	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, domainDBTypes, false, strmangle.SetComplement(domainPrimaryKeyColumns, domainColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	foreigners := []*DomainEmailUser{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, domainEmailUserDBTypes, false, strmangle.SetComplement(domainEmailUserPrimaryKeyColumns, domainEmailUserColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*DomainEmailUser{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddDomainEmailUsers(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.DomainEmailUsers[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.DomainEmailUsers[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.DomainEmailUsers().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
...
func testDomainToManyAddOpUserDomains(t *testing.T) {
	var err error
	...
	var a Domain
	var b, c, d, e UserDomain

	seed := randomize.NewSeed()
	if err = randomize.Struct(seed, &a, domainDBTypes, false, strmangle.SetComplement(domainPrimaryKeyColumns, domainColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	foreigners := []*UserDomain{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, userDomainDBTypes, false, strmangle.SetComplement(userDomainPrimaryKeyColumns, userDomainColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*UserDomain{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddUserDomains(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.UserDomains[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.UserDomains[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.UserDomains().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
		if want := int64((i + 1) * 2); count != want {
