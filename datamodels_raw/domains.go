...
var DomainRels = struct {
	DomainEmailAliases string
	DomainEmailUsers   string
	UserDomains        string
}{
	DomainEmailAliases: "DomainEmailAliases",
...
type domainR struct {
	DomainEmailAliases DomainEmailAliasSlice `boil:"DomainEmailAliases" json:"DomainEmailAliases" toml:"DomainEmailAliases" yaml:"DomainEmailAliases"`
...
func (r *domainR) GetDomainEmailAliases() DomainEmailAliasSlice {
	if r == nil {
		return nil
	}
	return r.DomainEmailAliases
}

func (r *domainR) GetDomainEmailUsers() DomainEmailUserSlice {
	if r == nil {
		return nil
	}
	return r.DomainEmailUsers
}

func (r *domainR) GetUserDomains() UserDomainSlice {
	if r == nil {
		return nil
	}
	return r.UserDomains
}
...
// DomainEmailAliases retrieves all the domain_email_alias's DomainEmailAliases with an executor.
func (o *Domain) DomainEmailAliases(mods ...qm.QueryMod) domainEmailAliasQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"domain_email_alias\".\"domain_id\"=?", o.ID),
	)

	return DomainEmailAliases(queryMods...)
}
...
func (o *Domain) DomainEmailUsers(mods ...qm.QueryMod) domainEmailUserQuery {
	...
	return DomainEmailUsers(queryMods...)
}
...
func (o *Domain) UserDomains(mods ...qm.QueryMod) userDomainQuery {
	...
	return UserDomains(queryMods...)
}

// LoadDomainEmailAliases allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (domainL) LoadDomainEmailAliases(ctx context.Context, e boil.ContextExecutor, singular bool, maybeDomain interface{}, mods queries.Applicator) error {
	var slice []*Domain
	var object *Domain
	...
	query := NewQuery(
		qm.From(`domain_email_alias`),
		qm.WhereIn(`domain_email_alias.domain_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err != nil {
		return errors.Wrap(err, "failed to eager load domain_email_alias")
	}

	var resultSlice []*DomainEmailAlias
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice domain_email_alias")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on domain_email_alias")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for domain_email_alias")
	}
	...
	if singular {
		object.R.DomainEmailAliases = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &domainEmailAliasR{}
			}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.DomainID {
				local.R.DomainEmailAliases = append(local.R.DomainEmailAliases, foreign)
				if foreign.R == nil {
					foreign.R = &domainEmailAliasR{}
				}
...
// AddDomainEmailAliases adds the given related objects to the existing relationships
// of the domain, optionally inserting them as new records.
// Appends related to o.R.DomainEmailAliases.
// Sets related.R.Domain appropriately.
func (o *Domain) AddDomainEmailAliases(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*DomainEmailAlias) error {
	var err error
	for _, rel := range related {
		if insert {
			...
			values := []interface{}{o.ID, rel.DomainID, rel.Address}
	...
	if o.R == nil {
		o.R = &domainR{
			DomainEmailAliases: related,
		}
	} else {
		o.R.DomainEmailAliases = append(o.R.DomainEmailAliases, related...)
	}

	for _, rel := range related {
		if rel.R == nil {
			rel.R = &domainEmailAliasR{
				Domain: o,
			}
		} else {
			rel.R.Domain = o
